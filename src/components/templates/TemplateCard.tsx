...
export function TemplateCard({ template, onEdit, onToggleActive, eligibleCount = 0 }: TemplateCardProps) {
  return (
    <div
      className={cn(
        "glass-card rounded-xl p-5 transition-all",
        template.active ? "hover:border-primary/30" : "opacity-60 border-dashed",
      )}
    >
      <div className="flex items-start justify-between mb-4">
        ...
        <div className="flex gap-1">
          ...
          <Button
            ...
            className={cn(
              "h-8 w-8",
              template.active ? "text-destructive hover:text-destructive" : "text-primary hover:text-primary",
            )}
      ...
      <div className="bg-muted/50 rounded-lg p-4 border border-border">
        <pre className="text-sm text-foreground/80 whitespace-pre-wrap font-sans">{template.content}</pre>
      </div>

      <div className="mt-4 flex items-center justify-between text-xs text-muted-foreground">
        <span>
          Variável: <code className="bg-muted px-1 rounded">{"{nome}"}</code>
        </span>
