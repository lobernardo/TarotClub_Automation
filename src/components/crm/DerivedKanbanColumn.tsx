/**
 * Kanban Column for Derived Stages
 * Uses derived CRM stages for proper grouping (lead_captured + agent_captured -> Lead Capturado)
 */
...
interface DerivedKanbanColumnProps {
  stage: DerivedCRMStage;
  leads: LeadWithDerivedStage[];
  onLeadClick?: (lead: Lead) => void;
  onLeadDrop?: (leadId: string, fromStage: LeadStage, toStage: DerivedCRMStage) => void;
}
...
export function DerivedKanbanColumn({ stage, leads, onLeadClick, onLeadDrop }: DerivedKanbanColumnProps) {
  const config = DERIVED_STAGE_CONFIG[stage];
      ...
      <div className="mb-4 px-1">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            <div className={cn("w-1 h-5 rounded-full", DERIVED_STAGE_ACCENTS[stage] || "bg-accent")} />
