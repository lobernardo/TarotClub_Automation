 ...
 * - Lead data (name, email, whatsapp, source, stage, notes)
...
const EVENT_LABELS: Record<string, { label: string; icon: React.ReactNode }> = {
  ...
  group_invite_sent: { label: "Link do grupo enviado", icon: <User className="h-4 w-4 text-teal-500" /> },
  group_join_confirmed: { label: "Entrada no grupo confirmada", icon: <CheckCircle className="h-4 w-4 text-green-500" /> },
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                {lead.source && (
                  <div className="flex items-center gap-2 text-sm">
                    ...
                    <span className="text-foreground">Origem: {SOURCE_LABELS[lead.source] || lead.source}</span>
                  </div>
                )}
                      ...
                      {timelineItems.map((item, index) => {
                        if (item.type === "event") {
                          ...
                          return (
                            ...
                            >
                              <div className="flex-shrink-0 mt-0.5 text-muted-foreground">{config.icon}</div>
