...
import { Calendar, Mail, MessageSquare, ClipboardList, ArrowDownUp } from "lucide-react";
...
function applyMonthlyReset(state: BoardState, now = new Date()): BoardState {
...
function getPaidAt(client: Lead): Date {
  const paidAt = (client as Lead & { asaas_paid_at?: string | null }).asaas_paid_at;
...
export default function ActiveClientsCRM() {
  ...
  useEffect(() => {
    const initialState = applyMonthlyReset(loadBoardState());
    setBoardState(initialState);
    saveBoardState(initialState);
  }, []);
  ...
  useEffect(() => {
    async function fetchClients() {
      ...
      const { data, error } = await supabase
        .from("leads")
        .select("id, name, email, whatsapp, stage, created_at, updated_at, asaas_paid_at")
  ...
  const clientsByStage = useMemo(() => {
    ...
    Object.values(grouped).forEach((list) => {
      list.sort((a, b) => getPaidAt(a).getTime() - getPaidAt(b).getTime());
    });
    ...
    <AppLayout>
      <div className="space-y-6 animate-fade-in">
        ...
        <div className="glass-card px-4 py-3 flex items-center gap-2 text-sm text-muted-foreground">
          ...
          <span>Ordem automática por data de pagamento (Asaas), do mais antigo para o mais novo.</span>
        </div>
                            ...
                            <div className="space-y-3">
                              ...
                              <div className="space-y-1 text-xs text-muted-foreground">
                                <div className="flex items-center gap-2">
                                  ...
                                  <span>
                                    Assinatura: {format(getPaidAt(client), "dd MMM yyyy 'às' HH:mm", { locale: ptBR })}
                                  </span>
                                </div>
                                {client.whatsapp && (
                                  <div className="flex items-center gap-2">
